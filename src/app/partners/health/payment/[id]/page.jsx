...
import { FaPhone, FaMapMarkerAlt, FaEnvelope, FaArrowLeft, FaHospital, FaCreditCard, FaLock, FaCheckCircle, FaRupeeSign, FaCopy, FaQrcode, FaGift, FaEye, FaTrash, FaDownload } from "react-icons/fa";
...
export default function HealthPaymentPage() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      ...
      router.push(`/login?redirect=/partners/health/payment/${id}`);
      return;
    }

    // Load consultation data from session storage
    const storedData = sessionStorage.getItem('healthConsultationData');
    if (storedData) {
      try {
        ...
        router.push(`/partners/health/consult/${id}`);
      }
    } else {
  ...
  const handleRazorpayPayment = async () => {
    try {
      ...
      const options = {
        ...
        description: `Consultation with ${partner?.name || 'Health Partner'}`,
        ...
        handler: async function (response) {
          try {
            ...
            try {
              ...
              } else if (couponError?.response?.data) {
                if (typeof couponError.response.data === 'string') {
                  errorMessage = couponError.response.data;
                } else if (couponError.response.data?.error) {
                  errorMessage = couponError.response.data.error;
                } else {
                  try {
                    const stringified = JSON.stringify(couponError.response.data);
                    if (stringified !== '{}') {
              ...
              console.error('Coupon generation error:', errorMessage);
              console.error('Error details:', {
                message: errorMessage,
                status: couponError?.response?.status,
                statusText: couponError?.response?.statusText,
                hasResponse: !!couponError?.response,
                responseDataType: typeof couponError?.response?.data
              });
      ...
      console.error('Razorpay payment error:', errorMsg);
      console.error('Error details:', {
        message: errorMsg,
    ...
    try {
      ...
      try {
        ...
        console.error('Full error details:', {
          message: errorMessage,
          response: couponError?.response?.data,
          status: couponError?.response?.status,
          error: couponError
        });
        toast.error(`Payment successful but coupon generation failed: ${errorMessage}`);
              ...
              <div className="mb-6">
                ...
                <div className="space-y-3">
                  ...
                  <label className="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors bg-yellow-50"
                         style={{ borderColor: paymentMethod === 'dummy' ? '#3b82f6' : '#fbbf24' }}>
                    <input
                      type="radio"
                      name="paymentMethod"
                      value="dummy"
              ...
              >
                              ...
                              >
                                ...
                                <td className="px-6 py-4">
                                  <div className="font-semibold text-gray-800 text-sm">
                                    {coupon.title || `Health Coupon ${indexOfFirstEntry + index + 1}`}
                                  </div>
                                </td>
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <p className="text-sm text-blue-800">
                    <strong>Note:</strong> Please save your coupon codes. You can use them at {partner?.name} to redeem your consultation.
                  </p>
                </div>
              </motion.div>
            )}
